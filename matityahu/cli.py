...
from matityahu.ingest.ascend import AscendImporter
...
from matityahu.reports.budget_vs_actual import (
    budget_vs_actual,
    flatten_budget,
)
...
def main():
    ...
    if args and args[0] == "import":
        if len(args) != 3:
            print("Usage: import [everydollar|ascend] <csv_path>")
            return
        
        source = args[1]
        path = Path(args[2])

        if source == "everydollar":
            importer = EveryDollarImporter()
        elif source == "ascend":
            importer = AscendImporter()
        else:
            print(f"Unknown import source: {source}.\nUse everydollar or ascend")
            return
    ...
    if args and args[0] == "report":
        ...
        loader = BudgetLoader(Path("config/budgets.yaml"))
        budget_raw = loader.get_month(year, month) or {}

        budget_data = flatten_budget(budget_raw)
        budget_categories = list(budget_data.keys())

        validate_categories(budget_categories, db.get_categories())
